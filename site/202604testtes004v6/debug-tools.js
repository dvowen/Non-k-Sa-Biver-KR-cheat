(() => {
  ...
  function setStatus(message) {
    const el = document.querySelector("[data-debug-tools-status]");
  ...
  function addGold(amount) {
    ...
    if (emit("add-coins", amount)) {
      setStatus(`${amount.toLocaleString("ko-KR")} 골드를 추가했습니다.`);
    }
  }

  function saveCurrentState() {
    if (emit("request-auto-save")) {
      setStatus("현재 상태 저장을 요청했습니다.");
    }
  }

  function setHpUnlimited(enabled) {
    state.hpUnlimited = enabled;
    if (state.hpTimer) {
      clearInterval(state.hpTimer);
      state.hpTimer = null;
    }
    if (enabled) {
      state.hpTimer = setInterval(() => {
        emit("update-player-hp", { current: 999999, max: 999999 });
  ...
  function changeBattleTime(seconds) {
    ...
    if (emit("debug-forward-time", seconds)) {
      setStatus(`${seconds > 0 ? "+" : ""}${seconds}초 사냥터 시간 조정을 요청했습니다.`);
    }
  }
  ...
  function grantSelectedItem() {
    ...
    const qty = document.querySelector("[data-debug-item-qty]");
    const itemId = select?.value;
    const count = readPositiveInteger(qty?.value, 1, 99);
    if (!itemId) {
      setStatus("아이템을 선택하세요.");
      return;
    }
    if (emit("add-item", { itemId, count })) {
      setStatus(`${itemId} ${count}개를 지급했습니다.`);
    }
  }
  ...
  function findSettingsContent(root) {
    return [...root.querySelectorAll("div")].find((el) => el.textContent.includes("설정은 자동으로 저장됩니다")) || root;
  }
  ...
  function ensureSettingsEnhancements() {
    ...
    if (!heading.dataset.debugUnlockBound) {
      ...
      heading.addEventListener("click", () => {
        ...
        if (state.settingsUnlockTapCount >= 5) {
          unlock();
          return;
        }
        if (state.settingsUnlockTapCount >= 3) {
          setSettingsHint(`${5 - state.settingsUnlockTapCount}번 더 탭하면 관리자 도구가 열립니다.`);
        }
      });
    }
